...
|                                                   |                                                                                                      |
|                                                   | 1.  119630 - A - Venus Karanja                                                                       |
|                                                   |                                                                                                      |
|                                                   | 2.  131038 - A - Doreen Muema                                                                        |
|                                                   |                                                                                                      |
|                                                   | 3.  135844 -A - Mercy Macharia                                                                       |
|                                                   |                                                                                                      |
|                                                   | 4.  104135 - B - Ryan Shuma                                                                          |
|                                                   |                                                                                                      |
|                                                   | 5.  ID - Group - Bill Akide                                                                          |
+---------------------------------------------------+------------------------------------------------------------------------------------------------------+
| **GitHub Classroom Group Name**                   | BEASTS                                                                                               |
+---------------------------------------------------+------------------------------------------------------------------------------------------------------+
| **Course Code**                                   | BBT4206                                                                                              |
+---------------------------------------------------+------------------------------------------------------------------------------------------------------+
...
purple_grey_colours_11 <- c("#E2B6CE", "#B9BCC2", "#888A8E", "#582340", "#E4E0C8",
                            "#D1B0E0", "#B191C9", "#A27DAF", "#8D6CAB", "#7B5E9A",
                            "#6A5290")

purple_grey_colours_6 <- c("#E2B6CE", "#B9BCC2", "#888A8E", "#A27DAF", "#8D6CAB", "#7B5E9A")
...
purple_grey_theme <- function() {
  theme(
    axis.ticks = element_line(
      ...
      lineend = NULL, color = "#582340"),  
    axis.text = element_text(
      face = "bold", color = "#582340",
      size = 12, hjust = 0.5),
    axis.title = element_text(face = "bold", color = "#582340",
      size = 14, hjust = 0.5),
    plot.title = element_text(face = "bold", color = "#582340",
      size = 16, hjust = 0.5),
    panel.grid = element_line(
      linewidth = 0.1, linetype = "dashed",
      lineend = NULL, color = "#582340"),
    panel.background = element_rect(fill = "#E4E1C8"), 
    legend.title = element_text(face = "plain", color = "#582340",
      size = 12, hjust = 0),
...
evaluation_per_group_per_gender <- student_performance_dataset %>%
  ...
  summarise(average_learning_rating =
              mean(`Average Level of Learning Attained Rating`, na.rm = TRUE)) %>%
  arrange(class_group, `Student's Gender`) %>%
  arrange(desc(average_learning_rating), .by_group = TRUE)

# Print the resulting data frame
#print(evaluation_per_group_per_gender)
...
evaluation_per_group_per_gender %>%
  ...
  mutate(`Average Level of Learning Attained Rating` =
           cell_spec(`Average Level of Learning Attained Rating`,
                     color = ifelse(`Average Level of Learning Attained Rating` >= 70, "green", "red"))) %>%
...
evaluation_per_group_per_gender %>%
  ...
  expand_limits(y = 0) +
  purple_grey_theme() +
  scale_fill_manual(values = purple_grey_colours_4) +
  ggtitle("Course Evaluation Learning Rating per Group and per Gender") +
...
color_bar_cell <- function(x) {
  cell_spec(x, color = "white", 
            background = scales::col_numeric(palette = c("#E2B6CE", "#E2B6CE"), domain = NULL)(x))
}
